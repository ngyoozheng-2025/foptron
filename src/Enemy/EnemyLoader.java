...
public class EnemyLoader {
    ...
    public static ArrayList<Enemy> loadEnemies(String filename, String difficulty, ArenaView arena, int mapRows,
            int mapCols) {
        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                Enemy template = createEnemyFromName(name, color, speed, handling, aggression, xpReward,
                        new Position(0, 0));
                if (template != null) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (difficulty.equalsIgnoreCase("EASY")) {
                ...
                if (roll < 10)
                    nameToSpawn = "Koura"; // Rare free kill
                else if (roll < 30)
                    nameToSpawn = "Sark";
                else if (roll < 70)
                    nameToSpawn = "Rinzler";
                else
                    nameToSpawn = "Clu"; // High chance of boss
            } else if (difficulty.equalsIgnoreCase("IMPOSSIBLE")) {
                if (roll < 20)
                    nameToSpawn = "Rinzler"; // Some Rinzler for variety
                else
                    nameToSpawn = "Clu"; // Mostly Clu (impossible boss)
            } else {
                // Default / Medium
                if (roll < 50)
                    nameToSpawn = "Sark";
