...
public class Disc {
    ...
    public void update(ArenaView arena, Characters player, ArrayList<Enemy> enemies) {
        ...
        // Check collisions at current position (both when moving to new tile and at current position)
        ...
        // 4. Out of range (only check when we've moved to a new tile to avoid premature deactivation)
     ...
     */
    public double getSmoothRow() { return smoothRow; }
    public double getSmoothCol() { return smoothCol; }
    
    /**
     * Check if collision glow is active
     */
    public boolean hasCollisionGlow() { return collisionGlowTimer > 0; }
    
    /**
     * Get collision glow intensity (0.0 to 1.0)
     */
    public float getCollisionGlowIntensity() {
        if (collisionGlowTimer <= 0) return 0.0f;
